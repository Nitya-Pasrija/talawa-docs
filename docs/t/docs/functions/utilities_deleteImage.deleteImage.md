deleteImage | talawa-apidocument.documentElement.dataset.theme = localStorage.getItem("tsd-theme") || "os"
...
*   [deleteImage](utilities_deleteImage.deleteImage.html)

Function deleteImage
====================

*   deleteImage(imageToBeDeleted, imageBelongingToItem?): Promise<void\>[](#deleteImage)
*   This function deletes an image if it is only used once. It is also ensured that the image hash isn't used by multiple users/organization before deleting it After deleting the image, the number of uses of the hashed image are decremented by one.
    ...
    *   ##### `Optional` imageBelongingToItem: string
    ...
    *   Defined in [src/utilities/deleteImage.ts:12](https://github.com/Nitya-Pasrija/talawa-api/blob/1d500f2/src/utilities/deleteImage.ts#L12)
